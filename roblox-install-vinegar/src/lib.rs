...
impl RobloxStudio {
    ...
    #[cfg(target_os = "linux")]
    fn locate_plugins_on_windows() -> Result<PathBuf> {
        let mut plugin_dir = dirs::data_local_dir().ok_or(Error::PluginsDirectoryNotFound)?;
        let home_dir = dirs::home_dir()
            .ok_or(Error::PluginsDirectoryNotFound)?;

        let current_user = home_dir
            .file_name()
            .ok_or(Error::PluginsDirectoryNotFound)?;

        plugin_dir.push("vinegar");
        plugin_dir.push("prefixes");
        plugin_dir.push("studio");
        plugin_dir.push("drive_c");
        plugin_dir.push("users");
        plugin_dir.push(current_user);
        plugin_dir.push("AppData");
        plugin_dir.push("Local");
        plugin_dir.push("Roblox");
        plugin_dir.push("Plugins");
    ...
    #[cfg(target_os = "linux")]
    fn locate_target_specific() -> Result<RobloxStudio> {
        let mut vinegar_root = dirs::data_local_dir().ok_or(Error::NotInstalled)?;
        vinegar_root.push("vinegar");
        Self::locate_from_directory(vinegar_root)
    }
    ...
    fn locate_from_windows_directory(root: PathBuf) -> Result<RobloxStudio> {
        ...
        if content_folder_path.is_dir() {
            if !plugins.is_dir() {
                fs::create_dir_all(&plugins).map_err(|_| Error::PluginsDirectoryNotFound)?;
            }
            ...
            let mut versions = root.join("Versions");
